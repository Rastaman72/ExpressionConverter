...
import Foundation
extension String {
    mutating func convertPrefixToInfix() {
        ...
        for element in stringToConvert {
            if mathOperator.keys.contains(element) == false {
                ...
                if elementToUse.count < 2 {
                    ...
                    var element1 = elementToUse[elementToUse.count - 1]
                    var element2 = elementToUse[elementToUse.count - 2]
    ...
    mutating func convertPrefixToPostfix() {
        self.convertPrefixToInfix()
        if self != "ERROR" {
